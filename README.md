...
1. **Root Planner** (Phase 1) — анализирует кодовую базу (`read_file`, `list_dir`, `shell_exec`) и декомпозирует работу на задачи через `create_task`
2. **Approval** (Phase 2) — показывает план пользователю: `[Y]es / [N]o / [A]mend`. При `auto_approve: true` пропускается автоматически
3. **Workers** (Phase 3) — пул горутин параллельно выполняет задачи, пишет код, запускает тесты
4. **Subplanners** — обрабатывают крупные задачи (`is_subplan: true`), разбивая их на подзадачи
5. Информация течёт **снизу вверх** через Handoff-документы
6. Planner переоценивает ситуацию, создаёт follow-up задачи, и завершает работу сигналом `CODEAGENTS_DONE`
