...
Planner получает `read_file`, `list_dir`, `shell_exec` для разведки кодовой базы перед планированием — чтобы создавать обоснованные задачи с правильным scope и constraints. Subplanner работает только с task tools.

Planner имеет доступ только к `create_task` -- это принуждает его к роли координатора, а не исполнителя.
